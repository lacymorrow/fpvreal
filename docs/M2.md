...
## The second run, on otto, 2026-09-15

Same clip, same section, on the Windows box with the RTX 3090
(`pipeline/run-scene.cmd`, started through `schtasks` so it outlives the
SSH session). Thirteen minutes end to end against four hours on the Mac.

| stage | otto | Mac |
|---|---|---|
| features and matching | 30 s, CUDA SIFT | 21 min |
| mapping | 76 s, 197 of 250 frames | 33 min |
| brush | 30,000 steps in 11 min, 67 steps a second | 15,000 steps in 2 h 46 min |
| mesh with carving | 108 s | |

The sharper splat exposed the real problem with the collision mesh. A
Gaussian splat fills free air with haze, and inside this bando the haze the
pilot flew through was as dense as the floor slab (23 against 26 units of
opacity per cubic metre, measured). No threshold separates them. What does
is evidence of free space, and the reconstruction has plenty: every sparse
point was seen from its cameras, so the segment from each camera to each of
its points is empty, and the flown path is a tube the quad fitted through.
Both are carved from the voxel grid before marching cubes. On this scene
219,113 sightlines removed 39% of the "solid" voxels, all of it haze, and
the pad search found the floor at the pilot's lowest point at once.

![The quad on the bando floor from the otto splat: brick walls, a window, the roof trusses, the sky](m2-hole-in-one.jpg)
...
large), and nobody has flown it with a radio yet.
